...
class AMSAdaptor {
    ...
    static async getFacesCard(skillsWriter: Object, faces: Faces, duration: ?number, containerSasUrl: string) {
        const facesData = [];
        let unknownFacesCounter = 1;
        const relevantFaces = slice(faces, 0, this.getMostNumberOfRelevantFaces());
        /* eslint-disable no-restricted-syntax, no-await-in-loop */
        for (const face of relevantFaces) {
            facesData.push({
                type: 'image',
                text: face.name || `Unknown #${unknownFacesCounter}`,
                image_url: this.getAzureThumbnailURL(containerSasUrl, face.thumbnailId),
        ...
        /* eslint-disable no-restricted-syntax, no-await-in-loop */
        return skillsWriter.createFacesCard(facesData, duration);
    }
    ...
    static async getSkillMetadataCards(skillsWriter: Object, insightsDetails: InsightDetails) {
        ...
        const facesCard = [await this.getFacesCard(skillsWriter, faces, duration, containerSasUrl)];
        const cards = [
            ...facesCard,
